    ...
    private boolean compute(){
        if(operate.equals("＋")){
            ...
            if(next_num.equals("0")){
                Toast.makeText(this,"除数不能为0哦~",Toast.LENGTH_SHORT).show();
                first_num = "";
                next_num = "";
                operate = "";
                return false;
    ...
    public void onClick(View v) {
        ...
        if (resID == R.id.btn_equal) {
            ...
            if (expression.equals("")) {
                Toast.makeText(this, "请先输入数字叭~", Toast.LENGTH_SHORT).show();
                datelist2.clear();
            ...
            expression = res_showNum.getText().toString();
            if (expression.equals("")) {
                Toast.makeText(this, "没有可以删除的数字啦~", Toast.LENGTH_SHORT).show();
            } else {
            ...
            if (expression.contains("＋") || expression.contains("－") || expression.contains("×") || expression.contains("÷")) {
                // 如果表达式中包含运算符，则报错
                Toast.makeText(getApplicationContext(), "不能有运算符", Toast.LENGTH_SHORT).show();
            } else {
                // 将表达式中的数字取相反数，并显示在文本框中
            ...
            showText = expression;
            if (expression.equals("")) {
                Toast.makeText(this, "请先输入数字可以莫~", Toast.LENGTH_SHORT).show();
            } else {
                for (String str : expression.split("[\\＋\\－\\×\\÷]")) {
                    datelist2.add(str);
                }
                if (datelist2.size() >= 1) {
                    ...
                    if (resID == R.id.btn_sin) {
                        ...
                        if (datelist2.get(datelist2.size() - 1).contains(".")) {
                            Toast.makeText(this, "阶乘不能是小数", Toast.LENGTH_SHORT).show();
                        } else
                            showText = String.valueOf(factBig(Integer.valueOf(datelist2.get(datelist2.size() - 1))));
                    } else if (resID == R.id.btn_squareroot) {
                        if (String.valueOf(Math.sqrt(Double.parseDouble(datelist2.get(datelist2.size() - 1)))) != "NaN") {
                            showText = String.valueOf(Math.sqrt(Double.parseDouble(datelist2.get(datelist2.size() - 1))));
                        } else showText = "根号内不能为负数哦~";
                    }
                    res_showNum.setText(showText);
                } else {
                    Toast.makeText(this, "请先输入数字好不好呀~", Toast.LENGTH_SHORT).show();
                }
            }
        } else {
            expression = res_showNum.getText().toString();
